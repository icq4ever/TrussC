...
//                     [--lang en|ko|ja|all|en,ko] [--docset-version <str>]
//                     [--dry] [--strict]
//
// --docset-version: TrussC version shown in the docset (default: `git describe`
//         in the repo, or $TRUSSC_VERSION). Displayed on the index + every footer.
//
...
// TrussC version the docset was built from: `git describe` in the repo, or an
// explicit --docset-version / $TRUSSC_VERSION override (e.g. for CI/tarballs).
const { execSync } = require('child_process');
function trusscVersion() {
    const ov = argVal('--docset-version') || process.env.TRUSSC_VERSION;
    if (ov) return ov.trim();
    try {
        return execSync('git describe --tags --always', { cwd: path.join(REF_DIR, '..', '..') })
            .toString().trim();
    } catch { return 'unknown'; }
}
const VERSION = trusscVersion();
...
<footer>TrussC <strong>${esc(VERSION)}</strong> · offline docset · built ${esc(BUILD_DATE)} · generated by emit-dash.js</footer>
</body></html>`;
}
...
// ---- index.html (landing) ----
{
    ...
    let body = `<h1>TrussC Reference <span class="ver">${esc(VERSION)}</span></h1><p class="lede">Offline API reference &amp; guides for TrussC ${esc(VERSION)} (built ${esc(BUILD_DATE)}). Use the search field, or browse below.</p>`;
...
h1 .ver{font-size:15px;font-weight:600;color:var(--muted);vertical-align:middle;border:1px solid var(--border);border-radius:5px;padding:1px 7px;margin-left:6px}
...
// ============================================================================
async function main() {
    console.log(`emit-dash: TrussC ${VERSION} — ${all.length} public symbols → ${index.length} index entries, ${files.size} files (lang: ${LANGS.join(',')})`);
    const byType = {};
